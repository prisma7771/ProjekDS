...
Buat ngambil tweet indo tags #Covid19
Ga usah dipake kalo mau import file kemarin
...
rstats_tweets <- search_tweets(q = "#Covid19",
                               ...
                               lang = "id")
```
Dipake kalo mau nyimpan tweetnya ke bentuk csv

```{r}
write_as_csv(rstats_tweets, here("covi19_tweet.csv"), prepend_ids = TRUE, na = "",fileEncoding = "UTF-8")
```

Yang penting kayaknya cuma teks, source sma hashtagsga sih?
...
data_tidy
```
Ngehitung jumlah masing-masingtags
```{r}
tagstidy <- data_tidy %>%
  select(tags) %>%
  group_by(tags) %>%
